...
left join Activated_Final A on M.week_date = A.earliest_activated_date and M.GMV = A.GMV
left join New_Logos L on L.start_date = M.week_date and M.GMV = L.GMV
left join MRR_ASP P on M.week_date = P.start_date and M.GMV = P.GMV
left join Workable W on M.week_date = W.workable_date and M.GMV = W.GMV
left join Flips F on  M.week_date = F.flip_date and M.GMV = F.GMV)
